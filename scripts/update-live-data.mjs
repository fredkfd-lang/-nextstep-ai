...
async function load(kind){
  ...
  for(let page = 1; page <= 10; page++){
    const p = new URLSearchParams({
      ...
      zeitarbeit: 'true',
      was: kind === 'ausbildung' ? 'Ausbildung' : 'Job'
    });
    ...
    for(const o of rows){
      ...
      const key = ref || [
        o.beruf,
        o.stellenangebotsTitel,
        o.arbeitgeber,
        typeof o.arbeitsort === 'object' ? o.arbeitsort?.ort : o.arbeitsort
      ].join('|');
      ...
      all.push({
        ...
        place: typeof o.arbeitsort === 'object'
          ? (o.arbeitsort.ort || o.arbeitsort.region || 'Deutschland')
          : (o.arbeitsort || o.ort || 'Deutschland'),
...
(async()=>{
  ...
  const ausbildung = await load('ausbildung');
  const jobs = await load('job');

  if(ausbildung.count < 6 || jobs.count < 6){
    throw new Error(`BA returned too few live records: Ausbildung=${ausbildung.count}, Jobs=${jobs.count}`);
  }
